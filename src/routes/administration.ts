...
import { IMMUNIZATION_DEFAULT_FHIR_URL } from '../config';
...
router.post('/immunization', async (req: Request, res: Response) => {
  try {
    ...
    const url = `${IMMUNIZATION_DEFAULT_FHIR_URL}/Immunization`;
...
router.post('/medication', async (req: Request, res: Response) => {
  try {
    ...
    const url = `${IMMUNIZATION_DEFAULT_FHIR_URL}/MedicationAdministration`;
